...
export async function upsertTrafficSource(data: {
  nm_id: number;
  date: string;
  source_name: string;
  ...
  const sql = `
    INSERT INTO traffic_source_analytics
      (nm_id, date, source_name, open_card_count, add_to_cart_count,
...
export async function getTrafficSourcesByNmId(
  ...
  let sql = 'SELECT * FROM traffic_source_analytics WHERE nm_id = ?';
