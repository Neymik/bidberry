...
export async function upsertStockSnapshot(stock: WBStock, snapshotDate: string): Promise<void> {
  const sql = `
    INSERT INTO stock_snapshots (nm_id, last_change_date, supplier_article, tech_size, barcode,
...
export async function upsertStocksBatch(stocks: WBStock[]): Promise<number> {
  const snapshotDate = dayjs().format('YYYY-MM-DD');
  let count = 0;
  for (const stock of stocks) {
    try {
      await upsertStockSnapshot(stock, snapshotDate);
      count++;
    } catch (e) {
...
export async function getStocksByNmId(nmId: number, snapshotDate?: string): Promise<DBStockSnapshot[]> {
  let sql = 'SELECT * FROM stock_snapshots WHERE nm_id = ?';
  ...
  if (snapshotDate) {
    ...
    sql += ' AND snapshot_date = (SELECT MAX(snapshot_date) FROM stock_snapshots WHERE nm_id = ?)';
    params.push(nmId);
  }
...
export async function getStocksSummary(snapshotDate?: string): Promise<any[]> {
  const dateCondition = snapshotDate
    ? 'snapshot_date = ?'
    : 'snapshot_date = (SELECT MAX(snapshot_date) FROM stock_snapshots)';
...
export async function getLatestSnapshotDate(): Promise<string | null> {
  const rows = await query<any[]>('SELECT MAX(snapshot_date) as latest FROM stock_snapshots');
